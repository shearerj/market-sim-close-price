...
@gin.configurable
class DQNAgent(TensorFlowAgent):
    """Double Deep Q Learning Agent."""
    ...
    def _policy(
        ...
        zi_id = tf.where(explore_cond, x=zi_id_explore, y=network_choice)

        params_chosen = tf.gather(self.zi_configs, zi_id)
        ...
        price, side, size, rmin, rmax, thresh = zi_chosen.policy(
            is_training,
            final_fundamental_estimate,
            private_bid,
            private_ask,
...
class QNetwork(snt.Module):
    ...
    def __init__(self, name=None):
        ...
        self.layers = tf.keras.Sequential(
            [
                tf.keras.layers.InputLayer(input_shape=(27,)),  # Still expects a batch dimension
