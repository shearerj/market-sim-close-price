...
from market_manipulation.utils.zscore_normalize import ZScoreNormalization

from .tensorflow_agent import BASE_POLICY_INPUT_TENSOR_SPEC, TensorFlowAgent


@gin.configurable
class DDPGAgent(TensorFlowAgent):
    ...
    def __init__(
        ...
        self.zscore_norm = ZScoreNormalization(pop_stats_path)
    ...
    def _policy(
        ...
        surplus_demand = tf.math.multiply(tf.cast(side, dtype=tf.float32), act) * self.action_coefficient
...
class Critic(snt.Module):
    def __init__(self, num_nodes: int, name=None):
    ...
    def __call__(self, observation, action):
        ...
        Args:
            observation:
        """
        output = self.critic_layers(tf.concat([observation, action], axis=1))
        return output
