...
@gin.configurable
class ZIMAgent(tfa.TensorFlowAgent):
    ...
    def __init__(
        ...
        self.benchmarkImpact = benchmarkImpact
    ...
    def _policy(
        ...
        benchmarkOffset = tf.multiply(tf.cast(side, dtype=tf.float64), self.benchmarkImpact)
        benchSurplus = tf.subtract(demandedSurplus, benchmarkOffset)
        ...
        toSubmit = self.shadePrice(self.threshold, side, bid_price, ask_price, estimatedValue, benchSurplus)
