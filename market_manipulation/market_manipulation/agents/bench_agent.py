"""Deep Determinisic Policy Gradient Learning Agent.
...
from market_manipulation.utils.zscore_normalize_bench import \
    ZScoreNormalizationBench

from .tensorflow_agent import BASE_POLICY_INPUT_TENSOR_SPEC, TensorFlowAgent


@gin.configurable
class BenchAgent(TensorFlowAgent):
    ...
    def __init__(
        ...
        self.benchmark_offset = benchmark_offset
    ...
    def _policy(
        ...
        if self.buffer_size < self.min_replay_size:
            act = tf.random.uniform(shape=[], minval=-1.0, maxval=1.0, dtype=tf.float32)
        ...
        action_offset = self.action_coefficient / 2 + self.benchmark_offset
        surplus_demand = tf.math.multiply(tf.cast(side, dtype=tf.float32), act) * action_offset
        surplus_demand += self.action_coefficient / 2
        # estimated_value = final_fundamental_estimate + private_benefit
        estimated_value = tf.add(tf.cast(final_fundamental_estimate, tf.float32), tf.cast(private_benefit, tf.float32))
        # price = estimated_value - (tf.math.multiply(tf.cast(side, dtype=tf.float64), act) * self.action_coefficient)
    ...
    def _training_step(self, transitions):
        ...
        # clipped_grads_actor, _ = clip_ops.clip_by_norm(grads_actor, self.clipped)
        ...
        # clipped_grads_critic, _ = clip_ops.clip_by_norm(grads_critic, self.clipped)
    ...
    def update(self) -> typing.Dict:
        ...
        update_log = {"actor_loss": actor_loss.numpy(), "critic_loss": critic_loss.numpy()}
...
@gin.configurable
class ActorBench(snt.Module):
    def __init__(self, num_actions: int, num_nodes: int, name=None):
        super(ActorBench, self).__init__(name=name)
    ...
    def __call__(self, observation):
        ...
        Args:
            observation:
        """
        output = self.actor_layers(observation)
        return output
