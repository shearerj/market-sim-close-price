...
if [[ "$(hostname)" == *'arc-ts.umich.edu' ]]; then
    ...
    MEM=$(( $(qstat "$PBS_JOBID" -M | tail -n1 | tr -s ' ' | cut -d' ' -f8 | rev | cut -c3- | rev) * 8 - 512))
fi
...
# Execution
java -ms1024m -mx${MEM}m -jar "$DIR/target/marketsim-4.0.0-jar-with-dependencies.jar" --no-features --jobs 1 --sims-per-obs "$SIMS_PER_OBS" --spec <(< "$1/simulation_spec.json" jq 'del(.configuration.numSims)' | jq -f "$DIR/jq/reduce_strats.jq") "$2" \
    | jq -c . \
