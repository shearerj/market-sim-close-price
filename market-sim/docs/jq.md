...
This filter takes as input an array of observation files (or the standard
output with the slurp (`-s`) option, and merges them into one observation file
with the average payoff for each player. Alternatively, one can group
observations together, in which case this can be used somewhat like numsims to
condense input.
