...
public class SummStats implements DoubleConsumer, Consumer<Entry<? extends Number>> {
	...
	public static SummStats over(Multiset<? extends Number> data) {
		return data.entrySet().stream().collect(SummStats::empty, SummStats::accept, SummStats::combine);
	}
	...
	public void combine(SummStats other) {
		...
		count += other.count;
		average += delta * other.count / count; // XXX not certain of proper order of operations ton minimize precision loss
	...
	public boolean equals(Object other) {
		...
		return this.count == that.count && this.average == that.average && this.max == that.max && this.min == that.min
				&& this.squaredError == that.squaredError;
	}
