...
public class RandomKeyedQueue<K, V> extends AbstractQueue<Entry<K, V>> {
	...
	protected OrderedQueue<V> getOrCreate(K time) {
		...
		if (q == null) {
			/*
			 * FIXME Do we want a constrained random queue, or a uniform random
			 * queue over deques. The constrianed random queue is nice because
			 * all orderings are uniform, but it also means scheduling more
			 * activities at the end makes your first activities more likely. A
			 * random queue would give equal weight to ever group of actions at
			 * the same time. I think we want the constrained one.
			 * 
			 * This can be thought of in the continuous time domain. The
			 * constrained random queue is like generating a random time for all
			 * of your activities and then sorting it and assigning the times of
			 * your activities that way. The random queue is like picking the
			 * time of your first activity, then rejection sampling your second
			 * activity until it's after the first, then the third until it's
			 * after the second and so on. Neither seems great...
			 */
