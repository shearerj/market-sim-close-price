 ...
 * 
 * A random queue (i.e. a queue where inputs come out in a random order) where
 * groups of items added together (with addAllOrdered) maintain their order
 * relationships when coming out, hence constrained.
 * 
 * The randomness is such that if you were to drain the queue, the distribution
 * over the order in which things came out would uniform over all feasible
 * orderings. For example if one added [1] and then added [2, 3] ordered, the possible
 * outputs from draining would be [1, 2, 3], [2, 1, 3], and [2, 3, 1]. Each
 * would occur with roughly equal probability. This also means that if you're
 * just removing one element, 2 is twice as likely as 1.
 */
