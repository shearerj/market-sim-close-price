...
/**
 * A priority queue that's based by a sorted set instead of Binary Heap. The
 * constants associated with most operations on this object are likely slower
 * than the corresponding java Binary Heap Priority Queue, but remove is O(log
 * n) instead of O(n) making cancellations much faster.
 * 
 * Like the java.util PriorityQueue, the minimum element has the highest
 * priority.
 */
