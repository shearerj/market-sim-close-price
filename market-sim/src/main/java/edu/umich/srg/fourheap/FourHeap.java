 ...
 * 
 * Unless noted, everything is constant time. `n` is the number of order
 * objects, not the quantity of objects i.e. adding more orders at the same
 * price doesn't increase the complexity.
 * 
 * @author ebrink
 * 
 * @param <P> Price
 ...
 * 
 * There are a few places where ? extends P is cast to P. For some reason
 * eclipse (not javac) complains about the comapareTo, and so this is cast to
 * end the complaining.
 * 
 */
public class FourHeap<P extends Comparable<? super P>> implements Serializable, Iterable<Order<P>> {
	...
	protected final Ordering<Order<P>> sellUnmatchedOrdering, sellMatchedOrdering, buyMatchedOrdering, buyUnmatchedOrdering;
	private int numUnits;
	private long time;

	public FourHeap() {
		// XXX This should just be natural().onResultOf(o -> o.price), but they need to be cast serializable, which breaks the eclipse compiler
		Ordering<Order<? extends P>> priceComp = new PriceOrdering(),
				timeComp = new TimeOrdering();

		// Note that default priority queue treats the minimum as the highest priority, so we reverse price here
	...
	public Order<P> submit(OrderType orderType, P price, int quantity) {
		...
		while (order.unmatchedQuantity > 0 // Quantity left to match
				...
				&& ((P) matchUnmatchedHeap.peek().price).compareTo(order.price) * order.type.sign() <= 0 // Can match with other order
				&& (orderMatchedHeap.isEmpty() || // Make sure it shouldn't kick out an order instead
		((P) matchUnmatchedHeap.peek().price).compareTo(orderMatchedHeap.peek().price) * order.type.sign() <= 0)) {
		...
		while (order.unmatchedQuantity > 0 // Quantity left to match
				...
				&& orderMatchedOrdering.compare(order, orderMatchedHeap.peek()) > 0) { // Should displace order
	...
	/**
	 * Withdraws a specific quantity from an order in the fourheap. Behavior is
	 * undefined if the order isn't already in the fourheap.
	 * 
	 * Complexity: O(n)
	 */
	...
	public boolean contains(Order<? extends P> order) {
		return sellUnmatched.contains(order)
				|| sellMatched.contains(order)
				|| buyUnmatched.contains(order)
				|| buyMatched.contains(order);
	}

	/**
	 * Clears matching orders from the fourheap, and returns a List of
	...
	public String toString() {
		return "<Bo: " + buyUnmatched + ", So: " + sellUnmatched + ", Bi: " + buyMatched + ", Si: " + sellMatched + ">";
	}
