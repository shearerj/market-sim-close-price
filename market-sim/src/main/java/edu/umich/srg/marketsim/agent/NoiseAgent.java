...
public class NoiseAgent implements Agent {
	...
	private static final IntUniform orderPriceDistribution = Uniform.discreteClosed(1000, 2000);
	...
	public static NoiseAgent createFromSpec(Sim sim, Fundamental fundamental, Collection<Market> markets, Market market, Spec spec, Random rand) {
	...
	private void strategy() {
		...
		market.submitOrder(
				orderTypeDistribution.sample(rand),
				Price.of(orderPriceDistribution.sample(rand)),
				quantityDistribution.sample(rand) + 1);
