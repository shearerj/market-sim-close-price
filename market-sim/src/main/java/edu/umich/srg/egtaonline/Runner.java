...
public class Runner {
  ...
  private static final Gson jsonReader = new Gson(),
      egtaWriter = new GsonBuilder().registerTypeAdapter(Player.class, new EgtaPlayerSerializer())
  ...
  public static void run(TriFunction<SimSpec, Log, Long, Observation> sim, Reader specs,
      Writer obsOut, Writer logOut, int numObs, int intLogLevel, boolean egta, String classPrefix,
    ...
    Stream<Enumerated<SimSpec>> specStream = Streams.stream(Iters
        .printExceptions(Iters.enumerate(Iters.repeat(Iters.map(fromJson(specs, JsonObject.class),
            (j) -> SimSpec.read(j, classPrefix, keyCaseFormat)), numObs))),
        true);
    specStream.map(c -> {
      try {
  ...
  public static void run(TriFunction<SimSpec, Log, Long, Observation> sim, String[] args,
    ...
    if (options.help.help) {
      ...
      try (Reader in = openin(options.simspec);
        ...
        run(sim, in, out, log, options.numObs, options.verbosity, options.egta, classPrefix,
            keyCaseFormat);
      }
    }
  }
