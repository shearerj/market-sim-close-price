 ...
 * "Sampling From the Binomial Distribution on a Computer" by George S. Fishman
 * (1979).
 * 
 * @author erik
 *
...
public abstract class Binomial implements LongDistribution, Serializable {
	...
	/*
	 * TODO The fact that these classes are public is an artifact of java 8 and
	 * should be fixed when java 9 allows these to be private. Currently they
	 * just muddle the API.
	 */
	...
	/**
	 * When n is small, a Binomial is best sampled by just generating n
	 * Bernoulli trials and summing the successes.
	 * 
	 * @author erik
	 *
	 */
	...
	/**
	 * If n is large, then we can use the Beta method for generating binomial
	 * samples. The idea of the method is simple. The jth order statistic of the
	 * sum of n Bernoulli trials is Beta(j, n - j + 1). Thus, if j = (n + 1) / 2
	 * and Y ~ Beta(j, j), then if Y < p at least j of the trials have to be
	 * larger than p (at least j successes). The conditional distribution for
	 * the trials that were less than Y but greater than p is also Binomial, and
	 * so we get a recursive definition that takes O(log n).
	 */
	...
	/**
	 * The Beta method works best when n is odd, thus if it's even, we do one
	 * independent Bernoulli trial, and add its result to the recursive
	 * definition.
	 */
