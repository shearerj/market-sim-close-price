...
public class Sip_Prime extends Entity {
	...
	private BestBidAsk lastQuote; // new -- best bid and ask for this market model
	...
	public BestBidAsk getNBBOQuote(int modelID) {
		/*old if (!lastQuotes.containsKey(modelID)) {
	...
	public ActivityHashMap updateNBBO(MarketModel model, TimeStamp ts) {
		...
		// new code -- should it be this.data.nbboLatency? 
		// THIS IS FOR NBBO SIP - TYPE THING!
		
		TimeStamp tsNew = ts.sum(this.data.nbboLatency);
		...
		//BestBidAsk lastQuote = computeBestBidOffer(ids, true); removed -- lastQuote already good
		...
		if ((bestBid != -1) && (bestAsk != -1)) {
			...
			if (lastQuote.bestBid > lastQuote.bestAsk) {
				...
				//old this.data.addNBBOSpread(modelID, ts, Consts.INF_PRICE);
				this.data.addNBBOSpread(modelID, tsNew, Consts.INF_PRICE); // new
			} else {
				// if bid-ask consistent, store the spread
				//old this.data.addNBBOSpread(modelID, ts, lastQuote.getSpread());
				this.data.addNBBOSpread(modelID, tsNew, lastQuote.getSpread()); // new
			}
		} else {
		...
		// lastQuotes.put(modelID, lastQuote); removed -- stored in lastQuote already
	...
	private BestBidAsk computeBestBidOffer(ArrayList<Integer> marketIDs, boolean nbbo) {
		// should be only markets in this model! not all markets
	    int bestBid = -1;
	    int bestBidMkt = 0;
	    int bestAsk = -1;
	    int bestAskMkt = 0;
