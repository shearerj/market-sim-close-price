...
public class AAAgent extends BackgroundAgent {
	...
	public Collection<Activity> agentStrategy(TimeStamp ts) {
		...
		ArrayList<Transaction> trans = new ArrayList<Transaction>(
				model.getTrans());
		findMovingAverage(movingAverage, mvgAvgNum, trans);

		// Determining Quantity
		int quantity = isBuyer ? 1 : -1;

		// Updating Price Limit
		limit = determinePriceLimit(quantity, ts);

		// Determine the Target Price
		Price targetPrice = new Price(-1);
		// Can only compute target price if movingAverage is valid
		if (!trans.isEmpty())
			targetPrice = determinePriceTarget(movingAverage);

		// Bidding Layer - only if at least one ask and one bid have been
		// submitted
		// See Vytelingum paper section 4.4 - Bidding Layer
		actMap.addAll(biddingLayer(targetPrice, quantity, ts));

		// Update the short term learning variable
		if (!trans.isEmpty()) {
			// Update the short term learning variable
			aggression = updateAggression(aggression, targetPrice, movingAverage);
			// Update the long term learning variable
			theta = updateAdaptiveness(theta, movingAverage, mvgAvgNum, trans);
		}
	...
	private void printInitDebugInfo() {
		...
		System.out.println("historical = " + historical);
		System.out.println("beta_r = " + beta_r);
	 ...
	 * @return aggression
	 */
	private double updateAggression(double aggression, Price tau,
			double movingAverage) {
		...
		return aggression + beta_r * (delta - aggression);
	}
	 ...
	 *            Current TimeStamp
	 */
	private double updateAdaptiveness(double theta, double movingAverage,
		...
		// Updating theta
		return theta + beta_t * (thetaStar - theta);
	}

	private Price determinePriceLimit(int quantity, TimeStamp ts) {
		Price fundPrice = model.getFundamentalAt(ts).times(Math.abs(quantity));
		Price deviation = privateValue.getValueFromQuantity(positionBalance, quantity);
	...
	private Collection<Activity> biddingLayer(Price targetPrice,
		...
		if (targetPrice.equals(new Price(-1))) {
			if (isBuyer) {
				price = new Price(bestBid.getPrice()
						+ (int) (min(bestAsk, limit).minus(bestBid).getPrice() / eta)
						+ 1);
				price = min(price, limit); // verifying
															// price <=
															// limit
			} else {
				price = new Price(bestAsk.getPrice()
						- (int) (bestAsk.minus(max(bestBid, limit)).getPrice() / eta)
						- 1);
				price = max(price, limit); // verifying
															// price >=
															// limit
			}
		}
		...
		// back) - unnecessary if eta is an int
			...
			if (isBuyer) {
				price = new Price(bestBid.getPrice() + (targetPrice.minus(bestBid)).getPrice() / eta + 1);
				price = min(price, limit);
			} else {
				price = new Price(bestAsk.getPrice() - bestAsk.minus(targetPrice).getPrice() / eta - 1);
