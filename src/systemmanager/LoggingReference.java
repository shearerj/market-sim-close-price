 ...
 * 
 * There are two use cases: EGTA analysis and Market models simulation. I) EGTA
 * analysis - In this use case, only ONE market model will be created. This is
 * specified by the primary market model in the spec file. - Model-level agents
 * are not created. Both environment agents & players will be created. II)
 * Market models simulation - In this case, multiple market models can be
 * specified. - Anything in the "assignment" section of the spec file will be
 * ignored. - No players will be created. Both environment and model-level
 * agents will be created. - Set of types of model-level agents does NOT include
 * any agents specified in the configuration section of the spec file (i.e.
 * environment agents).
 * 
 * Strategies: Sets up and assigns strategies (assumed to follow format of
 * parameter- value pairs separated by underscores: [param]_[value]_...
 * 
 * Sequence of initialization: - generate background arrival times - generate
 * background private valuations - create market models - create the markets -
 * setup the markets - add Clear activities, if needed - create the agents -
 * environment agents, model-level agents, or players - common random numbers
 * for agents in each market model - agent-specific parameters are added to
 * ObjectProperties - add AgentArrival activities - set agent permission for
 * each model's markets
 * 
 * NOTE: Usage of the primary model setting in the spec file: 1)
 * MODELNAME-CONFIG 2) MODELNAME (will use first config in that model's list to
...
//				}
			...
			// No players means market models simulation; no primary model (set
			// as null)
	...
	private String[] parseModelConfigList(String modelConfigList) {
		...
		if (modelConfigList != null) {
			if (modelConfigList.endsWith(",")) {
				...
				modelConfigList = modelConfigList.substring(0,
						modelConfigList.length() - 1);
			}
		...
		if (data.isEGTAUseCase()) {
			...
			for (AgentPropsPair app : data.getPlayerMap().keySet()) {
				...
				if (playerNumbers.containsKey(type)) {
					...
					playerArrivalGenerators.put(type, new ArrivalTime(
							new TimeStamp(0), data.arrivalRate));
	...
	private void createNonPlayersForModel(MarketModel model) {
		try {
			for (AgentPropsPair ap : allNonPlayers.keySet()) {
				...
				ArrayList<Integer> assignMktIDs = assignAgToMkts(numAg, null /*
																			 * model
																			 * .
																			 * getMarketIDs
																			 * (
				...
				// check if (1) is model agent, (2) present in this model OR if
				// environment agent
				if ((data.getModelAgentMap().containsKey(ap) && data.getModelAgentByModel(
						model.getID()).contains(ap))
...
//								assignMktIDs.get(i).toString());
					...
					Agent ag = createAgent(model,
							...
							playerArrivalGenerators.get(type).next());
	...
	/**
	 * Create an agent with the specified properties & type in the given market
	 * model.
	 * 
	 * @param model
	 * @param ap
...
//					data.simLength));
	...
	/**
	 * Given a number of agents and a number of market IDs, splits them as
	 * evenly as possible. Returns an array of length numAgents, each element
	 * being one of the valid marketIDs. The distribution is as even as
	 * possible.
	 * 
	 * @param numAgents
	 * @param mktIDs
