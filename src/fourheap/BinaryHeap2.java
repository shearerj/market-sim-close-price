 ...
 */
public class BinaryHeap2<E> extends AbstractQueue<E> implements Serializable {
	private static final long serialVersionUID = 1999398475077245857L;

	protected final List<E> queue;
	protected final Ordering<? super E> comparator;
	protected final Map<E, Integer> locations;
	protected transient int modCount = 0;

	protected BinaryHeap2(int initialCapacity, Comparator<? super E> comparator) {
	...
	public static <E extends Comparable<? super E>> BinaryHeap2<E> create() {
		return create(Ordering.natural());
	}
	
	public static <E> BinaryHeap2<E> create(Comparator<? super E> comparator) {
	...
	public static <E> BinaryHeap2<E> create(int initialCapacity, Comparator<? super E> comparator) {
		return new BinaryHeap2<E>(initialCapacity, comparator);
	}
	...
	public static <E> BinaryHeap2<E> create(
			Iterable<? extends E> initialElements,
			Comparator<? super E> comparator) {
		
		BinaryHeap2<E> queue = create(comparator);
	...
	protected void siftDown(int k, E x) {
		...
		while (k < half) {
			...
			E c = queue.get(child);
			int right = child + 1;
			if (right < size() && comparator.compare( c, (E) queue.get(right)) > 0)
	...
	protected final class HeapIterator implements Iterator<E> {
		...
		public void remove() {
			...
			if (lastRet >= 0) {
				E moved = BinaryHeap2.this.removeAt(lastRet);
				lastRet = -1;
				if (moved == null)
					cursor--;
				else
				...
				BinaryHeap2.this.removeEq(lastRetElt);
			}
