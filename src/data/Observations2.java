...
public class Observations2 {
	...
	public Observations2(SimulationSpec spec, Collection<MarketModel> models) {
	...
	protected static JsonObject getTransactionInfo(MarketModel model,
		...
		for (Transaction t : model.getTrans()) {
			...
			transPrices.add(tr.execTime, (double) tr.price.getPrice());
			fundPrices.add(tr.execTime,
					(double) model.getFundamentalAt(tr.execTime).getPrice());
	 ...
	 * 
	 * FIXME Is this correct? Which side do the NaN's go on?
	 */
	protected static JsonObject getSpread(MarketModel model, long maxTime) {
		...
		for (Market market : model.getMarkets()) {
			TimeSeries s = new TimeSeries(); // data.marketSpread.get(mktID);
												// TODO fix spread calculation
			if (s != null) { // FIXME should never happen
		...
		TimeSeries nbbo = new TimeSeries(); // data.NBBOSpread.get(model.getID());
											// TODO fix spread calculation
		if (nbbo != null) { // FIXME should never happen
	...
	protected static JsonObject getVolatility(MarketModel model, int period, long maxTime) {
		...
		for (Market market : model.getMarkets()) {
			...
			TimeSeries mq = new TimeSeries(); // data.marketMidQuote.get(mktID); FIXME move to market
			if (mq != null) { // FIXME should never happen
				...
				feat.addProperty(delimit("_", prefix, STDDEV, LOG + RETURN, suffix), logStdev);
