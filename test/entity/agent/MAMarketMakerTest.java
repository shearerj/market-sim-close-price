...
public class MAMarketMakerTest {
	...
	// TODO check that bid ask filled up correctly
	
	// TODO check that bid ask when full is evicted appropriately
	...
	public void quoteChangeTest() {
		...
		// market maker with window of length 3
		MAMarketMaker marketmaker = new MAMarketMaker(fundamental, sip, market, 
				new Random(), setupProperties(2, 10, false, 1, 2));
		...
		em.executeUntil(new TimeStamp(1));
		// Check that market maker queues updated
		assertEquals("Bid queue incorrect", new Price(40), marketmaker.bidQueue.peek());
		assertEquals("Ask queue incorrect", new Price(50), marketmaker.askQueue.peek());
		...
		// Check that market maker queues updated
		assertEquals("Bid queue incorrect", marketmaker.bidQueue.contains(new Price(42)));
//		assertEquals("Ask queue incorrect", marketmaker.askQueue.contains(new Price(48)));
		assertEquals("Bid queue incorrect", new Price(42), marketmaker.bidQueue.peek());
		...
		// Check market maker orders to be average of bid/asks
		Order order = orders.get(8);
		assertEquals(marketmaker, order.getAgent());
		assertEquals(new Price(31), order.getPrice());
		assertEquals(OrderType.BUY, order.getOrderType());
		order = orders.get(9);
		assertEquals(marketmaker, order.getAgent());
		assertEquals(new Price(41), order.getPrice());
		assertEquals(OrderType.BUY, order.getOrderType());
		...
		order = orders.get(11);
		assertEquals(marketmaker, order.getAgent());
		assertEquals(new Price(49), order.getPrice());
		assertEquals(OrderType.SELL, order.getOrderType());
	}
	
	// TODO advanced quote change test with window length 3
