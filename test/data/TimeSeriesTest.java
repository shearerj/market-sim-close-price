...
public class TimeSeriesTest {
	...
	public void lengthTest() {
		...
		assertEquals(51, array.length);
		
		t = new TimeSeries();
		array = t.sample(3, 100);
		assertEquals(34, array.length);
		...
		for (int i = 0; i < 100; i++) {
			...
			assertEquals(length / period + 1, array.length);
		}
	}
	...
	public void truncationTest() {
		...
		t.add(100, 3.6);
		array = t.sample(1, 100);
		for (double d : Arrays.copyOfRange(array, 0, 100))
			assertEquals(5.6, d, 0);
		assertEquals(3.6, array[100], 0);
	}
	...
	public void sansNansTest() {
		...
		t = new TimeSeries();
		t.add(50, 5.6);
		array = TimeSeries.sansNans(t.sample(1, 100), 3.4);
		for (double d : Arrays.copyOfRange(array, 0, 50))
			assertEquals(3.4, d, 0);
		for (double d : Arrays.copyOfRange(array, 50, 101))
			assertEquals(5.6, d, 0);
	...
	public void logRatioTest() {
		...
		array = TimeSeries.logRatio(t.sample(1, 3));
