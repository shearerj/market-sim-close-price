...
\end{verbatim}
will create a zipped folder \path{market_maker_sim.zip}. The zipped simulator folder will include the basic configuration (i.e., the \path{env.properties} file) specified in the \path{egta} directory as well as the necessary \path{batch} script for running simulations on the cluster.

Note that you do not explicitly specify the path for the \path{defaults.json} file. Instead, the script will use \path{config/<simulator_name>.json}.
In the example above, \path{config/market_maker_sim.json} will be copied into the zipped folder as \path{defaults.json}.
This \path{json} file stores the simulator's default configurations, which will be the default values filled in whenever a new scheduler is created using your simulator.
Format-wise, this \path{json} file looks exactly like the \path{simulation_spec.json} but with the ``assignment'' section removed.


\begin{quote}
\textbf{Logging simulator versions:} Since each simulator's \path{defaults.json} file can vary widely depending on what environment parameters are needed, be sure to commit and push your updated simulator configuration files, all stored within the \path{config/} directory. In addition, whenever you upload a new simulator or update an existing simulator, \emph{tag} the commit in the git repository. Tag format is as follows:
\begin{verbatim}
<simulator_name>-<version><upload_label>
\end{verbatim}
For example, \verb|market_maker_sim-2.4a| is the first tagged commit for version 2.4 of the \path{market_maker_sim} simulator. Any subsequent updates to this simulator will have incremented labels, e.g. \verb|market_maker_sim-2.4b|.
...
\item[\texttt{Zipped Source}:] Select the zipped folder you have just created. The name of the \path{.zip} file should match the simulator name, i.e. you should be selecting \path{market_maker_sim.zip}
\end{description}
...
\end{description}
\end{quote}
The fields following \texttt{Simulator} will automatically update with the fields specified in \path{defaults.json}. If using non-default values, be very careful to avoid typos; otherwise, your simulation data will not be stored correctly in the database.
